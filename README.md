...
Для корректной работы скрипта в файле скрипта в переменной `project_directory` необходимо указать полный путь до проекта на сервере, например:
```bash
...
Чтобы отключить запрос пароля при перезапуске сервисов systemd в файле `/etc/sudores` необходимо добавить строку:
```shell
username ALL=(ALL) NOPASSWD: /user/bin/systemctl daemon-reload
```
