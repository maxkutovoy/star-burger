...
Создайте файл `.env` в корневом каталоге проекта и пропишите в нем настройки проекта в формате `КЛЮЧ=ЗНАЧЕНИЕ`:
...
Создайте файл `.env` в корневом каталоге проекта (если еще этого не сделали) и пропишите в нем настройки в формате `КЛЮЧ=ЗНАЧЕНИЕ`:
...
Создайте файл `.env` в корневом каталоге проекта, если еще этого не сделалаи и задайте переменные окружения (какие переменныре указыать см. ["Задайте переменные окружения"](#задайте-переменные-окружения)).

**ВНИМАНИЕ!**
Для подключения к локальной базе данных в файле `.env` в настройке `DATABASE_URL` значение `host` укажите `host.docker.internal`.
```sh
DATABASE_URL=postgres://username:password@host.docker.internal:5432/db_name
```
...
Сайт будет доступен на странице  [http://127.0.0.1/](http://127.0.0.1/)

### Запуск prod-версии сайта:

Для работы prod-версии сайта необходимо получить сертификат безопасности, для этого в файле `.env` добавьте настройки:
